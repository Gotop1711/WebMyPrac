Vue.component('myCalendar', {
    template: `<div class="calendar-wrap">
            ...
            <div class="calendar-body">
                <div class="calendar-vue-day_type" v-show="panelType == 'day'">
                    ...
                    <ul class="calendar-body-box">
                        <li v-for="(item, index) in dateList" :key="index" v-text="item.value"
                            :class="{pre: item.previousMonth, next: item.nextMonth, curry: date === item.value && month === tmpMonth && item.currentMonth, choose: nowValue === item.value && item.currentMonth}"
                ...
                <div class=" calendar-vue-month_type" v-show="panelType == 'month'">
                    <ul class="calendar-body-box">
                        <li v-for="(item, index) in monthList" :key="index" v-text="item.label"
                            :class="{choose:item.value == tmpMonth}" @click="selectMonth(item)"></li>
                    </ul>
                </div>
                <div class="calendar-vue-year_type" v-show="panelType == 'year'">
                    <ul class="calendar-body-box">
                        <li v-for="(item,index) in yearList" :key="index" v-text="item"
                            :class="{choose:item === tmpYear && fixedYear === 0}" @click="selectYear(item)"></li>
                    </ul>
                </div>
            </div>
        </div>`,
    props: {
        parValue: {
            default: function () {
                return { showYear: new Date().getFullYear(), showMonth: new Date().getMonth() + 1, showDay: new Date().getDate() }
            }
        }
    },
    data() {
    ...
    computed: {
        ...
        yearList() {
            ...
            let curryYearList = Array.from({ length: 12 }, (value, index) => curryYear * 12 + index);
